...
CREATE TABLE IF NOT EXISTS `login`.`Cases` (
  ...
  `avg_time` float(64) COLLATE utf8_unicode_ci  COMMENT 'num slides in case',
  PRIMARY KEY (`case_id`)
...
CREATE TABLE IF NOT EXISTS `login`.`Interviews` (
  ...
  `timeTaken` float(64) COLLATE utf8_unicode_ci  COMMENT 'num slides in case',
  PRIMARY KEY (`interview_id`)
...
CREATE TABLE IF NOT EXISTS `login`.`Slides` (
  ...
  `slide_pdf` varbinary(MAX) COLLATE utf8_unicode_ci NOT NULL COMMENT 'actual slide pdf contents ',
  PRIMARY KEY (`case_id`)
